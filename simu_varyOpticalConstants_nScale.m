...
% Varied n spectra (real part of refractive index)
nSpectra = zeros(size(nReal,1),length(nScale));

% Photogenerations from modelled optics
StoreTotGphPred = zeros(length(desiredIlluminance),length(filenameSpectra),length(ActiveMATthicknesses),length(nScale),length(EgActiveMATref));
StoreGphPred = zeros(length(filenameSpectra),length(ActiveMATthicknesses),length(lambda),length(nScale));
StoreGphVsPosition = cell(length(filenameSpectra),length(ActiveMATthicknesses),length(nScale),length(EgActiveMATref));

% Normalized (device) optics
StoreReflection = zeros(length(ActiveMATthicknesses),length(nScale),length(lambda));
StoreAbsorption = zeros(length(ActiveMATthicknesses),length(nScale),length(lambda),length(materials));
StoreTransmission = zeros(length(ActiveMATthicknesses),length(nScale),length(lambda));

%% Simulation

% Loop over different n spectra
nInitial = nReal(:,indActiveLayer+1);
for idxnScale = 1:length(nScale)
    % Progress report
    disp(['nScale: ',num2str(nScale(idxnScale)),' (', ...
        num2str(idxnScale),'/',num2str(length(nScale)),')'])
    tStartnScale = tic;

    nSpectra(:,idxnScale) = nScale(idxnScale)*nInitial;
    nReal(:,indActiveLayer+1) = nSpectra(:,idxnScale);
    ...
    while idxActiveMATt <= length(ActiveMATthicknesses)
        try
            ...
            StoreReflection(idxActiveMATt,idxnScale,:) = R;
            StoreAbsorption(idxActiveMATt,idxnScale,:,:) = A;
            StoreTransmission(idxActiveMATt,idxnScale,:) = T;
                ...
                for idxIfactor = 1:length(IntensityFactors{idxSpectrum})
                    ...
                    for idxEg = 1:length(EgActiveMATref)
                        ...
                        if ismembertol(idxIfactor,length(IntensityFactors{idxSpectrum}),1e-5)
                            ...
                            StoreGphVsPosition{idxSpectrum,idxActiveMATt,idxnScale,idxEg} = ...
    ...
    % Progress report
    % tElapsed = toc(tStart);
    tElapsednScale = toc(tStartnScale);
    % disp(['Total runtime so far: ', num2str(tElapsed/(60)),' min']);
    disp(['Runtime of last nScale: ', num2str(tElapsednScale/60),' min']);
end
